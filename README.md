...
Detailed documentation for contributors and researchers is available in the [`docs/`](docs/) folder:

| Document | Description |
|----------|-------------|
| [Repository Guide](docs/repository-guide.md) | Complete reference for every file and folder — start here if you're new |
...
This repository uses a **Zipformer-Small (~30M parameters)** encoder with an RNN-T transducer head. The architecture is configured entirely through defaults in [`ASR/zipformer/train.py`](ASR/zipformer/train.py).

| Component | Configuration |
|-----------|---------------|
| Encoder stacks | 6 (U-Net shaped) |
| Encoder dims | 192, 256, 256, 256, 256, 256 |
| Layers per stack | 2, 2, 2, 2, 2, 2 |
| Frame rates | 50, 25, 12.5, 6.25, 12.5, 25 Hz |
| Attention heads | 4, 4, 4, 8, 4, 4 |
| Total parameters | ~30M |

See [docs/model-30m-configuration-guide.md](docs/model-30m-configuration-guide.md) for a complete parameter reference, constraint rules, and guidance on how to change the model size.
